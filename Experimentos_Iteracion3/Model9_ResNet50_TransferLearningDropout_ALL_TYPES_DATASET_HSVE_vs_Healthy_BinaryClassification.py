...
from keras.layers import Conv2D, Flatten, Dense, Input, AveragePooling2D, Dropout, MaxPooling2D, BatchNormalization, Activation # capas para añadir
...
class_HSVE_path = './DATASETS/Dataset_Model9/HSVE_ORIGINAL_ALL_TYPES_RESCALED_256/' # Ubicación remota las imágenes MRI de la clase positiva (cerebros con la enfermedad HSVE)
...
class_HealthyBrains_path = './DATASETS/Dataset_Model9/HEALTHY_BRAINS_ALL_TYPES_RESCALED_256/' # Ubicación remota las imágenes MRI de la clase negativa (cerebros sanos)
...
plot_model(base_model, to_file='arquitecturas_modelos/Model9_baseModel_ResNet50.png', show_shapes=True)
...
with tf.device('/GPU:0'):
    ...
    # Save the model defined previously
    plot_model(model_gpu, to_file='arquitecturas_modelos/Model9_ResNet50_TransferLearningDropout_ALL_TYPES_DATASET_arquitecture.png', show_shapes=True)
...
class EarlyStoppingCallback(Callback):
  def on_epoch_end(self, epoch, logs=None):
    if logs.get('val_accuracy') >= 0.85:
      print("El val_accuracy alcanzó el valor 0.85. Deteniendo el entrenamiento.")
      self.model.stop_training = True

# Se instancia el callback de Early Stopping
early_stopping_val_accuracy_85 = EarlyStoppingCallback()
...
with tf.device('/GPU:0'):
    ...
    h = model_gpu.fit(X_train, train_targets_array, epochs = 30, validation_data=(X_val, val_targets_array), batch_size = 8, callbacks=[early_stopping_val_accuracy_85 , early_stop_val_accuracy, early_stop_val_loss], verbose=1) # training method in which data and targets are passed with some specific parameters
...
df.to_csv('classification_reports/Model9_ResNet50_TransferLearningDropout_ALL_TYPES_DATASET_classification_report.csv', index=True)
...
fig.savefig('figuras/Model9_ResNet50_TransferLearningDropout_ALL_TYPES_DATASET_confusion_matrix.png')
...
fig.savefig('figuras/Model9_ResNet50_TransferLearningDropout_ALL_TYPES_DATASET_accuracy_vs_loss.png')
...
fig = figure.get_figure()
fig.savefig('figuras/Model9_ResNet50_TransferLearningDropout_ALL_TYPES_DATASET_accuracy.png')
...
fig = figure.get_figure()
fig.savefig('figuras/Model9_ResNet50_TransferLearningDropout_ALL_TYPES_DATASET_loss.png')
...
# activations.savefig('figuras/Model9_ResNet50_TransferLearningDropout_ALL_TYPES_DATASET_20_activations')
...
np.save('modelos_entrenados/Model9_ResNet50_TransferLearningDropout_ALL_TYPES_DATASET_HSVE_vs_Healthy_ResNet50_HISTORY.npy',h.history)
...
model_gpu.save('modelos_entrenados/Model9_ResNet50_TransferLearningDropout_ALL_TYPES_DATASET_HSVE_vs_Healthy_ResNet50.h5')
